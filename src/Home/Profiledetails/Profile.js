...
function Profile() {
    ...
    const fetchData = async () => {
      try {
        const registerResponse = await axios.get(
          `http://192.168.1.165:9090/register/getById/${values1}`
        );
        setValues(registerResponse.data);
        console.log(values.email+"this is values");

        const paymentResponse = await axios.get(
          `http://192.168.1.165:9090/payment/getCustomerDetailsByMail/${values1}`
  ...
  const handleSendOtp = async () => {
    ...
    const proxyUrl = `http://192.168.1.165:9090/register/sendOtp?mobileno=${mobileno}&otp=${otp}`;
    try {
  ...
  const handleSendEmailOtp = async () => {
    ...
    const proxyUrl = `http://192.168.1.165:9090/register/sendEmail/${useremail}`;
    try {
  ...
  const handleOtpSubmit = async () => {
    if (otp === generatedOtp) {
      try {
        await axios.put(
          `http://192.168.1.165:9090/register/user/update/${values.email}`,
          {
            ...values,
  ...
  const handleDownloadInvoice = async (policyId) => {
    try {
      const response = await axios.get(`http://192.168.1.165:9090/payment/create?userId=${policyId}`, {
